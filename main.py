...
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_r:
                ...
                ex = input("End X: ")
                ey = input("End Y: ")
                while (sx == ex and sy == ey) or int(sx) > 16 or int(sx) < 0 or int(sy) > 16 or int(sy) < 0 or int(ex) > 16 or int(ex) < 0 or int(ey) > 16 or int(ey) < 0:
    ...
    if reset:
        ...
        startNode = grid[int(sx)][int(sy)]
        startNode.itsObstacle = False
        startNode.itsStart = True
        #goalNode    = grid[rows-3][cols//2]
        goalNode    = grid[int(ex)][int(ey)]
        goalNode.itsObstacle = False
        goalNode.color = (0, 0, 255)
        goalNode.itsDestination = True
